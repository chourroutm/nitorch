...
- [ ] T005 [P] [US1] Test that `make_image(dat, anatomix=<valid local checkpoint path>)` produces an `ImagePyramid` whose levels' `dat` has `output_nc` channels and whose `.preview` holds the original intensities, in `nitorch/tests/test_anatomix_image.py`.
- [ ] T006 [P] [US1] Test that selecting an existing loss (e.g. `make_loss('lcc')`) on `anatomix`-transformed fixed/moving images runs through the existing pairwise registration workflow end-to-end and produces a spatial transform, in `nitorch/tests/test_anatomix_image.py` (contracts/anatomix-api.md §5, spec.md US1 Acceptance Scenario 2).
- [ ] T007 [P] [US1] Regression test that `make_image(dat, anatomix=None)` (the default) produces byte-for-byte identical output to calling `make_image` without the parameter at all, in `nitorch/tests/test_anatomix_image.py` (FR-008).
- [ ] T008 [P] [US1] Automated SC-002 comparison test in `nitorch/tests/test_anatomix_image.py`: construct a synthetic multimodal pair (a base volume and a contrast-transformed copy — e.g. intensity-inverted or nonlinearly remapped — with a small known ground-truth deformation applied), register it both with `anatomix=None` (intensity-based) and with a valid `anatomix=` weights path, score both results with an existing nitorch registration-quality metric (e.g. `losses.LCC` against the known-good alignment, or `losses.Dice` if label maps are used), and assert the anatomix-based result scores better. Automates quickstart.md Scenario 2's "Validation against SC-002" (spec.md SC-002).

### Implementation for User Story 1

- [ ] T009 [US1] Add the `anatomix=None` parameter to `make_image()` in `nitorch/tools/registration/pairwise_makeobj.py`, normalizing its `None`/`str`/`True`/`dict` forms into an `AnatomixFeatureExtractor` config per research.md §5 and data-model.md's `anatomix=` configuration table. Depends on T004.
- [ ] T010 [US1] Wire per-pyramid-level extraction into `make_image()`'s existing `for level in image:` loop (the same loop `mind` uses): `level.preview = level.dat` then `level.dat = extract_features(level.dat, **config)`, in `nitorch/tools/registration/pairwise_makeobj.py` (research.md §1, §6). Depends on T009.
- [ ] T011 [P] [US1] Add the `--anatomix [PATH]` flag to the per-image options block in `nitorch/cli/registration/register/parser.py`, mirroring `--mind [FWHM=1 [RADIUS=0]]` (contracts/anatomix-api.md §3).
- [ ] T012 [US1] Forward the parsed `anatomix` value to `make_image()` by adding it to the existing keyword-forwarding tuple in `nitorch/cli/registration/register/cli.py:296` (`('soft', 'bound', 'extrapolate', 'mind')` → `(..., 'anatomix')`). Depends on T009, T011.
- [ ] T013 [US1] Run T005-T008, confirm they pass, and manually walk through quickstart.md Scenarios 2 and 3.
